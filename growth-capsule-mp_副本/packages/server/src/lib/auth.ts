...
const JWT_SECRET = process.env.JWT_SECRET || 'growth-capsule-dev-secret-change-in-production'

/**
 * Sign a JWT token containing the user's uid.
 */
 ...
 *   1. Authorization: Bearer <JWT>  (mini program / production)
 *   2. X-DEV-UID HTTP header        (curl / Postman / programmatic)
 *   3. dev-uid cookie                (browser dev toolbar)
 *   4. DEV_DEFAULT_UID env var       (server-wide default)
 *   5. Fallback: uid_default_local
 */
